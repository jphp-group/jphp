...
public class ReflectionProperty extends Reflection implements Reflector {
    ...
    public Memory getValue(Environment env, Memory... args) throws Throwable {
        ...
        if (arg.isNull()){
            ...
            return entity.getClazz().getStaticProperty(env, env.trace(), entity.getName(), true, false);
        } else {
            if (arg.isObject()){
                if (entity.isStatic())
    ...
    public Memory setValue(Environment env, Memory... args) throws Throwable {
        ...
        if (entity.isStatic()){
            ...
            entity.getClazz().getStaticProperty(env, env.trace(), entity.getName(), true, false).assign(args[1]);
        } else {
            if (arg.isObject()){
